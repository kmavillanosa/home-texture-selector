...
export function VisualizerPage() {
	...
	}, [detectionResult, selectedRegionId, setSelectedRegionId, visibleDetections])
	...
	return (
		<div className="flex min-h-0 flex-1 flex-col">
			...
			<div className="sticky top-0 z-20 flex shrink-0 items-center justify-between gap-4 border-b border-slate-200/60 bg-transparent px-4 py-3 shadow-none backdrop-blur-sm dark:border-slate-800/60">
				<Link
					to="/upload"
								...
								{visibleDetections.length === 0 ? (
									...
									<div className="flex flex-col gap-1">
										{visibleDetections.map((d) => {
											const isActive = d.label === selectedRegionId
											return (
												<button
													key={d.label}
													type="button"
								...
								scenes.map((scene, index) => {
									...
									return (
										<button
											...
											className={`group flex w-32 shrink-0 flex-col overflow-hidden rounded-xl border text-left transition-colors sm:w-40 ${
												isActive
										...
										>
											<div className="relative h-20 w-full overflow-hidden bg-slate-100">
												...
												{isActive && (
													<span className="absolute right-2 top-2 rounded-full bg-teal-600 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-white">
														Active
													</span>
												)}
											</div>
											<div className="px-2.5 py-2 text-xs font-medium text-slate-700 dark:text-slate-200">
