...
1. **Docker** (recommended for local dev):
   ...
   ```
   Then set in `api/segmentation-api/.env`:
   ```
...
Uploads use **Cloudflare R2** when configured (S3-compatible, no egress fees). Without config, files are stored in `api/segmentation-api/uploads/`.

1. Create an R2 bucket at [Cloudflare Dashboard](https://dash.cloudflare.com) → R2.
2. Create API tokens: R2 → Manage R2 API Tokens.
3. Add to `api/segmentation-api/.env` (see `api/segmentation-api/.env.example` for full list):
   ```
